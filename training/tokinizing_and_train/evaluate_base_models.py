...
def evaluate(model_path: str, data_path, label, cased: bool):
    model_name = model_path
    model = AutoModelForQuestionAnswering.from_pretrained(model_name)
    tokenizer = AutoTokenizer.from_pretrained(model_name)

    eval_data = _load_data_set(label, data_path)["evaluation_set"]
    ...
    saving_path = "./base_results/{}/{}".format(model_name, label)
...
def _load_data_set(label, data_path):
    with open("{}/data_set_split_{}.json".format(data_path, label), "r") as file:
...
if __name__ == '__main__':
    ...
    for model, c in [("deepset/minilm-uncased-squad2", False),
        ...
        for label in ["", "what", "which", "how"]:
            ...
            evaluate(model, data_path, label, c)
